-- All three marts here are one row per entity per season. A duplicate would
-- silently double-count a player or team on every leaderboard (or double a
-- team's win-loss record in the standings), so fail loudly instead. Written
-- as a singular test to avoid adding a dbt_utils dependency for one
-- assertion.
